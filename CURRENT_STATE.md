# Current State

Last updated: Fri 11/09/2026

## Website

Status: Live, but requires review and cleanup.

Known or suspected issues:

- navigation and site structure need review;
- some content may be unfinished or outdated;
- some product links may be broken;
- mobile and visual consistency need checking.

## Rehumanizer

Status: Partially built / needs inspection.

Known issue:

- deployment or linking may currently be broken.

## RFP Evaluator

Status: Prototype exists from earlier collaborative development.

Next step:

- inspect current implementation, deployment and user experience.

## Other experiments

Status: Inventory required.

## Current priority

Establish a safe baseline, verify backups, inspect the live site and identify the smallest high-value repair.
